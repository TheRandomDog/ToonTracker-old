...
class CustomCommandsModule(Module):
    ...
    class AddCMD(Command):
        ...
        async def execute(cls, client, module, message, *args):
            if not args:
                return module.create_discord_embed(subtitle='Please reference the command name.', color=Color.red())
            command_name = args[0]
            command_reply = ' '.join(args[1:])
            for command in module.commands:
                if command['name'] == command_name:
                    return module.create_discord_embed(subtitle='The command **{}** already exists.'.format(command_name), color=Color.red())
            ...
            Config.set_module_setting('custom_commands', 'commands', module.commands)
            module.load_command(command)
            return module.create_discord_embed(subtitle='Created the **{}** command.'.format(command_name), footer='Available Commands: ~removeCommand | ~viewCommand | ~editCommand', color=Color.green())
        ...
        async def execute(cls, client, module, message, *args):
            ...
            for command in module.commands:
                if command['name'] == command_name:
                    ...
                    return module.create_discord_embed(subtitle='Removed the **{}** command.'.format(command_name), color=Color.green())
            return module.create_discord_embed(subtitle='The command **{}** does not exist.'.format(command_name), color=Color.red())
    ...
    class EditCMD(Command):
        ...
        async def execute(cls, client, module, message, *args):
            ...
            if len(args) < 2:
                return module.create_discord_embed(subtitle='Please reference the part of the command to edit.', footer='Available Parts: name | description | action', color=Color.red())
            part = args[1]
            if part == 'name':
                if len(args) < 3:
                    return module.create_discord_embed(subtitle='Please provide the new name for the command.', color=Color.red())
                new_name = args[2]
                for c in module.commands:
                    if c['name'] == new_name:
                        return module.create_discord_embed(subtitle='The command **{}** already exists.'.format(new_name), color=Color.red())
                ...
                if len(args) < 3:
                    return module.create_discord_embed(subtitle='Please edit an action by referring to its number or using `add`.', color=Color.red())
                ...
                if action_context.isdigit():
                    ...
                    if not (0 < action_context <= len(module.commands[command_index]['actions'])):
                        return module.create_discord_embed(subtitle='Please choose a number that represents an action.', color=Color.red())
                    if len(args) < 4:
                        return module.create_discord_embed(subtitle='Please refer what part you\'d like to modify of this action.', footer='Available Parts: dm | channel | content | delete', color=Color.red())
                    action_part = args[3]
                    if action_part == 'delete':
                        ...
                        if len(args) < 5:
                            return module.create_discord_embed(subtitle='Please use **true** / **false** if you\'d like the response to this command to be sent in a DM.', color=Color.red())
                        ...
                        if not message.channel_mentions:
                            return module.create_discord_embed(subtitle='Please use a channel mention to refer to a channel.', color=Color.red())
                        else:
                            module.commands[command_index]['actions'][action_context - 1]['channel'] = message.channel_mentions[0].id
                    else:
                        return module.create_discord_embed(subtitle='Unknown action part: **{}**'.format(action_part), footer='Available Parts: dm | channel | content | delete', color=Color.red())
                elif action_context == 'add':
                    if len(args) < 4:
                        return module.create_discord_embed(subtitle='Please mention what type of action you\'d like to add.', footer='Available Actions: Reply | Send', color=Color.red())
                    action_type = args[3]
                    if action_type.lower() == 'reply':
                        ...
                        if not message.channel_mentions:
                            return module.create_discord_embed(subtitle='Please use a channel mention to refer to the channel this action should be sent to.', color=Color.red())
                        else:
                            module.commands[command_index]['actions'].append({'action': 'Send', 'channel': message.channel_mentions[0].id, 'content': ' '.join(args[5:])})
                    else:
                        return module.create_discord_embed(subtitle='Unknown action: **{}**'.format(action_type), footer='Available Actions: reply | send', color=Color.red())
                else:
                    return module.create_discord_embed(subtitle='Please edit an action by referring to its number or using `add`.', color=Color.red())
            else:
                return module.create_discord_embed(subtitle='Invalid part: **{}**'.format(part), footer='Available Parts: name | description | action', color=Color.red())
            Config.set_module_setting('custom_commands', 'commands', module.commands)
            module.__init__(module.client)
            return module.visualize_command(module.commands[command_index])
    ...
    def __init__(self, client):
        ...
        self.commands = Config.get_module_setting('custom_commands', 'commands', [])
    ...
    def load_command(self, command):
        class CustomCommand(Command):
            ...
            RANK = assert_type(command.get('rank', 0), int, otherwise=0)
    ...
    def visualize_command(self, command):
        embed = Embed(title=self.client.command_prefix + command['name'], description=command.get('description', Embed.Empty), color=Color.from_rgb(114, 198, 255))
        actions = 0
        for action in command['actions']:
            actions += 1
            embed.add_field(
